...
void panel_container_insert(mapgen_panel_container *cont, int x, int y, int image, int depth, bool collision){
    ...
    if(cont[ad].size == PANEL_SEGMENTS_IN_DATA) return;

    for(int i=0; i<cont[ad].size; i++){
        if(depth == cont[ad].panel[i].depth) return;
    }
    cont[ad].panel[cont[ad].size].image = image;
    cont[ad].panel[cont[ad].size].depth = depth;
    cont[ad].panel[cont[ad].size].collision = collision;
    cont[ad].size++;
}
